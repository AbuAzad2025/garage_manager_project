...
def _json_loads(value: str) -> dict[str, Any]:
    ...
    try:
        ...
        current_app.logger.warning("Invalid JSON received: %s", value)
        return {}
...
def available_qty(product_id: int) -> int:
    ...
    reserved = (
        ...
        .select_from(OnlinePreOrderItem)
...
def catalog():
    ...
    products = (
        ...
        .having(func.coalesce(func.sum(StockLevel.quantity), 0) > 0)
...
def add_to_cart(product_id):
    ...
    if item:
        ...
        db.session.add(
            OnlineCartItem(cart_id=cart.id, product_id=product.id, quantity=qty, price=product.price or 0)
        )
    try:
        db.session.commit()
...
def update_cart_item(item_id):
    ...
    if new_qty > available_qty(item.product_id) + item.quantity:
                ...
                if prepaid > 0:
                    db.session.add(
                        Payment(
                            ...
                            method=(form.payment_method.data or 'online'),
...
def preorder_receipt(preorder_id):
    ...
    if preorder.customer_id != g.online_customer.id:
        abort(403)
...
def cancel_preorder(preorder_id):
    ...
    if po.customer_id != g.online_customer.id:
...
def inject_cart_count():
    if current_user.is_authenticated:
        cart = get_active_cart(getattr(current_user, 'id', None))

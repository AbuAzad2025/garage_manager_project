...
def _items_snapshot(sh):
    return [{"product_id": int(i.product_id or 0), "warehouse_id": int(i.warehouse_id or 0), "quantity": int(i.quantity or 0)} for i in sh.items]

@shipments_bp.route("/", methods=["GET"], endpoint="list_shipments")
@login_required
@permission_required("manage_warehouses")
def list_shipments():
    q = db.session.query(Shipment).options(joinedload(Shipment.items), joinedload(Shipment.partners).joinedload(ShipmentPartner.partner), joinedload(Shipment.destination_warehouse))
    ...
    if search:
        like = f"%{search}%"
        q = q.filter(or_(Shipment.shipment_number.ilike(like), Shipment.tracking_number.ilike(like), Shipment.origin.ilike(like), Shipment.carrier.ilike(like), Shipment.destination.ilike(like), Shipment.destination_warehouse.has(Warehouse.name.ilike(like))))
    page = request.args.get("page", 1, type=int)
    per_page = request.args.get("per_page", 20, type=int)
    pagination = q.order_by(Shipment.expected_arrival.desc().nullslast(), Shipment.id.desc()).paginate(page=page, per_page=per_page, error_out=False)
    if _wants_json():
        return jsonify({"data": [{"id": s.id, "number": s.shipment_number, "status": s.status, "origin": s.origin, "destination": (s.destination_warehouse.name if s.destination_warehouse else (s.destination or None)), "expected_arrival": s.expected_arrival.isoformat() if s.expected_arrival else None, "total_value": float(s.total_value or 0)} for s in pagination.items], "meta": {"page": pagination.page, "pages": pagination.pages, "total": pagination.total}})
...
def shipments_data():
    ...
    f_from   = _parse_dt(request.args.get("from") or "")
    f_to     = _parse_dt(request.args.get("to") or "")
    f_dest   = (request.args.get("destination") or "").strip()
    f_extra  = (request.args.get("search_extra") or "").strip()
    ...
    data = [{"id": s.id, "number": s.shipment_number, "status": s.status, "origin": s.origin, "destination": (s.destination_warehouse.name if s.destination_warehouse else (s.destination or None)), "expected_arrival": s.expected_arrival.isoformat() if s.expected_arrival else None, "total_value": float(s.total_value or 0)} for s in rows]
        ...
        for entry in getattr(form.partners, "entries", []):
            ...
            if f.partner_id.data:
                db.session.add(ShipmentPartner(
                    ...
                    notes=f.notes.data,
                    role=getattr(f, "role", None).data if hasattr(f, "role") else None,
                ))
        db.session.flush()
...
def edit_shipment(id: int):
    ...
    if form.validate_on_submit():
        ...
        sh.currency = _norm_currency(form.currency.data)
...
def shipment_detail(id: int):
    sh = _sa_get_or_404(Shipment, id, options=[joinedload(Shipment.items).joinedload(ShipmentItem.product), joinedload(Shipment.partners).joinedload(ShipmentPartner.partner), joinedload(Shipment.destination_warehouse)])
    try:
        from models import Payment, PaymentStatus
        total_paid = db.session.query(func.coalesce(func.sum(Payment.total_amount), 0.0)).filter(Payment.shipment_id == sh.id, Payment.status == PaymentStatus.COMPLETED.value).scalar() or 0.0
    except Exception:
        total_paid = sum(float(p.total_amount or 0) for p in getattr(sh, "payments", []))
    if _wants_json():
        return jsonify({"shipment": {"id": sh.id, "number": sh.shipment_number, "status": sh.status, "origin": sh.origin, "destination": (sh.destination_warehouse.name if sh.destination_warehouse else (sh.destination or None)), "expected_arrival": sh.expected_arrival.isoformat() if sh.expected_arrival else None, "total_value": float(sh.total_value or 0), "items": [{"product": (it.product.name if it.product else None), "warehouse_id": it.warehouse_id, "quantity": it.quantity, "unit_cost": float(it.unit_cost or 0), "declared_value": float(it.declared_value or 0), "landed_extra_share": float(it.landed_extra_share or 0), "landed_unit_cost": float(it.landed_unit_cost or 0), "notes": it.notes} for it in sh.items], "partners": [{"partner": (ln.partner.name if ln.partner else None), "share_percentage": float(ln.share_percentage or 0), "share_amount": float(ln.share_amount or 0)} for ln in sh.partners], "total_paid": float(total_paid or 0)}})

...
def q2(x)->Decimal: return D(x).quantize(TWOPLACES,rounding=ROUND_HALF_UP)

def _ensure_payment_number(pmt:Payment)->None:
    if getattr(pmt,"payment_number",None): return
...
def _sum_splits_decimal(splits)->Decimal:
    total=Decimal("0")
    for s in splits or []: total+=q2(getattr(s,"amount",0))
    return q2(total)

def _norm_currency(v): return (v or "ILS").strip().upper()
...
def _dir_to_db(v:str|None):
    vv=_norm_dir(v)
    if vv=="IN": return PaymentDirection.INCOMING.value
    if vv=="OUT": return PaymentDirection.OUTGOING.value
    return vv
...
def _clean_details(d:dict|None):
    ...
    for k,v in d.items():
        ...
        if isinstance(v,(date,datetime)): cleaned[k]=v.isoformat()
        else: cleaned[k]=str(v)
...
def _coerce_method(v):
    ...
    try: return PaymentMethod(s)
    except Exception: return PaymentMethod.CASH

def _sync_payment_method_with_splits(pmt:Payment):
    try: splits=list(pmt.splits or [])
    except Exception: splits=[]
    if not splits:
        if not getattr(pmt,"method",None): pmt.method=PaymentMethod.CASH
        return
    first=splits[0]
    new_m=getattr(first,"method",None)
    new_m_val=getattr(new_m,"value",new_m)
    if new_m_val and new_m_val!=getattr(pmt,"method",None): pmt.method=new_m_val
...
def _serialize_split(s):
    return {"id":s.id,"amount":float(q2(getattr(s,"amount",0) or 0)),"method":(getattr(getattr(s,"method",None),"value",getattr(s,"method","")) or ""),"details":(getattr(s,"details",None) or None)}

def _serialize_payment(p,*,full=False):
    d={"id":p.id,"payment_date":(p.payment_date.isoformat() if getattr(p,"payment_date",None) else None),"total_amount":float(q2(getattr(p,"total_amount",0) or 0)),"currency":getattr(p,"currency","ILS") or "ILS","method":(getattr(getattr(p,"method",None),"value",getattr(p,"method","")) or ""),"direction":(getattr(getattr(p,"direction",None),"value",getattr(p,"direction","")) or ""),"status":(getattr(getattr(p,"status",None),"value",getattr(p,"status","")) or ""),"entity_type":getattr(p,"entity_type","") or "","entity_display":p.entity_label() if hasattr(p,"entity_label") else (getattr(p,"entity_type","") or ""),"splits":[_serialize_split(s) for s in (list(getattr(p,"splits",[]) or []))]}
    if full:
        d.update({"payment_number":getattr(p,"payment_number",None),"receipt_number":getattr(p,"receipt_number",None),"reference":getattr(p,"reference",None),"notes":getattr(p,"notes",None),"customer_id":getattr(p,"customer_id",None),"supplier_id":getattr(p,"supplier_id",None),"partner_id":getattr(p,"partner_id",None),"sale_id":getattr(p,"sale_id",None),"invoice_id":getattr(p,"invoice_id",None),"service_id":getattr(p,"service_id",None),"expense_id":getattr(p,"expense_id",None),"preorder_id":getattr(p,"preorder_id",None),"shipment_id":getattr(p,"shipment_id",None),"loan_settlement_id":getattr(p,"loan_settlement_id",None)})
    return d
...
def _render_payment_receipt_pdf(payment:Payment)->bytes:
    ...
    try: return HTML(string=html,base_url=request.url_root).write_pdf(stylesheets=[CSS(string=css_inline)])
    except Exception:
...
def entity_search():
    ...
    def rows_for(model,extra_cols=("phone","mobile","email")):
        ...
        for c in extra_cols:
            ...
            if col is not None: conds.append(col.ilike(like))
    ...
    if t=="CUSTOMER":
        ...
        conds=[]
        if qdigits:
            try: conds.append(SupplierLoanSettlement.id==int(qdigits))
...
@permission_required("manage_payments")
def index():
    if not getattr(current_user,"is_authenticated",False): return redirect(url_for("auth.login",next=request.full_path))
    ...
    filters=[]
    if entity_type: filters.append(Payment.entity_type==entity_type)
    if status: filters.append(Payment.status==status)
    if direction: filters.append(Payment.direction==_dir_to_db(direction))
    if method:
        try:
    ...
    if sd: filters.append(Payment.payment_date>=datetime.combine(sd,time.min))
    if ed: filters.append(Payment.payment_date<=datetime.combine(ed,time.max))
    ...
    if reference_like: filters.append(Payment.reference.ilike(f"%{reference_like}%"))
    ...
    if _wants_json():
        return jsonify({"payments":[p.to_dict() for p in pagination.items],"total_pages":pagination.pages,"current_page":pagination.page,"total_items":pagination.total,"totals":{"total_incoming":float(total_incoming_d),"total_outgoing":float(total_outgoing_d),"net_total":float(net_total_d),"grand_total":float(grand_total_d),"total_paid":float(total_incoming_d)}})
    return render_template("payments/list.html",payments=pagination.items,pagination=pagination,total_paid=float(total_incoming_d),total_incoming=float(total_incoming_d),total_outgoing=float(total_outgoing_d),net_total=float(net_total_d),grand_total=float(grand_total_d))
...
def create_payment():
    ...
    if hasattr(form,"direction"): form.direction.choices=[("IN","وارد"),("OUT","صادر"),("INCOMING","وارد"),("OUTGOING","صادر")]
    if request.method=="GET":
        form.payment_date.data=datetime.utcnow()
        try:
            tokens=session.get("pmt_tokens") or []
            if len(tokens)>50: tokens=tokens[-30:]
            tok=uuid.uuid4().hex; tokens.append(tok); session["pmt_tokens"]=tokens
            if hasattr(form,"request_token"): form.request_token.data=tok
        except Exception:
            if hasattr(form,"request_token"): form.request_token.data=None
        ...
        if pre_amount<=0: pre_amount=D(request.args.get("total_amount"))
        if pre_amount<=0: pre_amount=None
            ...
            if et=="CUSTOMER" and eid:
                ...
                if s:
                    ...
                    if pre_amount is None:
                        pre_amount=abs(nb)
                        if nb>0: form.direction.data="OUT"
                        elif nb<0: form.direction.data="IN"
                    if pre_amount is not None:
                        form.total_amount.data=pre_amount
                ...
                if svc:
                    ...
                    entity_info={"type":"service","number":svc.service_number,"date":svc.request_date.strftime("%Y-%m-%d") if getattr(svc,"request_date",None) else "","balance_due":float(getattr(svc,"balance_due",0) or 0),"total":float(getattr(svc,"total_cost",0) or 0),"paid":float(getattr(svc,"total_paid",0) or 0),"currency":getattr(svc,"currency","ILS") if hasattr(svc,"currency") else "ILS"}
                ...
                if exp:
                    ...
                    entity_info={"type":"expense","number":f"EXP-{exp.id}","date":exp.date.strftime("%Y-%m-%d") if getattr(exp,"date",None) else "","description":exp.description or "","amount":float(getattr(exp,"amount",0) or 0),"balance":float(getattr(exp,"balance",0) or 0),"currency":getattr(exp,"currency","ILS"),"type_name":getattr(getattr(exp,"type",None),"name",None),"employee_name":getattr(getattr(exp,"employee",None),"name",None)}
        ...
        if preset_currency: form.currency.data=(preset_currency or "").upper()
        if preset_method and hasattr(form,"method"):
            try: form.method.data=_coerce_method(preset_method).value
            except Exception: form.method.data=(preset_method or "").lower()
        if preset_direction: form.direction.data=_norm_dir(preset_direction)
        if preset_ref: form.reference.data=preset_ref
        if preset_notes and hasattr(form,"notes"): form.notes.data=preset_notes
        if pre_amount is not None and not form.total_amount.data: form.total_amount.data=pre_amount
    ...
    if request.method=="POST":
        ...
        tokens=session.get("pmt_tokens") or []
        if req_tok and (req_tok in tokens): tokens=[t for t in tokens if t!=req_tok]; session["pmt_tokens"]=tokens
        else:
            msg="تم استلام هذه العملية مسبقًا أو انتهت صلاحية الجلسة."
            if _wants_json(): return jsonify(status="error",message=msg),409
            flash(msg,"warning"); return redirect(url_for("payments.index"))
        raw_dir=request.form.get("direction")
        if raw_dir: form.direction.data=_norm_dir(raw_dir)
        if not form.validate():
            current_app.logger.warning("payment.validate_failed",extra={"event":"payments.validate_failed","errors":dict(form.errors or {}),"path":request.path})
        ...
        try:
            ...
            if parsed_splits:
                ...
                if sum_splits!=q2(tgt_total_dec):
            ...
            _sync_payment_method_with_splits(payment); db.session.add(payment)
            try:
                db.session.commit()
            except IntegrityError as ie:
                db.session.rollback()
                msg=str(ie).lower(); fixed=False
                ...
                if "payments.method" in msg or "may not be null" in msg:
                    if parsed_splits: payment.method=getattr(parsed_splits[0].method,"value",parsed_splits[0].method)
                    else: payment.method=PaymentMethod.CASH
                    fixed=True
                if fixed:
            ...
            try:
                ...
                if payment.status==PaymentStatus.COMPLETED.value:
                    ...
                    if payment.loan_settlement_id:
                        ...
                        if ls and ls.supplier_id: update_entity_balance("supplier",ls.supplier_id)
                if payment.preorder_id:
                    ...
                    if po and payment.direction==PaymentDirection.INCOMING.value and payment.status==PaymentStatus.COMPLETED.value:
                        if hasattr(PreOrderStatus,"PAID"):
                            try: po.status=PreOrderStatus.PAID.value if hasattr(PreOrderStatus.PAID,"value") else PreOrderStatus.PAID; db.session.add(po)
                            except Exception: pass
                db.session.commit()
            ...
            db.session.rollback()
            current_app.logger.exception("payment.create_failed",extra={"event":"payments.create.error","path":request.path})
            ...
            if _wants_json(): return jsonify(status="error",message=str(e)),400
            flash(f"❌ خطأ في الحفظ: {e}","danger"); return render_template("payments/form.html",form=form,entity_info=entity_info)
        if _wants_json(): return jsonify(status="success",payment=_serialize_payment(payment,full=True)),201
...
def create_expense_payment(exp_id):
    ...
    if hasattr(form,"_entity_field_map") and "EXPENSE" in form._entity_field_map: getattr(form,form._entity_field_map["EXPENSE"]).data=exp.id
    entity_info={"type":"expense","number":f"EXP-{exp.id}","date":exp.date.strftime("%Y-%m-%d") if getattr(exp,"date",None) else "","description":exp.description or "","amount":float(q2(D(getattr(exp,"amount",0) or 0))),"currency":getattr(exp,"currency","ILS")}
    ...
    try:
        ...
        _ensure_payment_number(payment)
        for sp in parsed_splits: payment.splits.append(sp)
        db.session.add(payment); db.session.commit()
        log_audit("Payment",payment.id,f"CREATE (expense #{exp.id})")
        if _wants_json(): return jsonify(status="success",payment=_serialize_payment(payment,full=True)),201
        flash("✅ تم تسجيل دفع المصروف بنجاح","success"); return redirect(url_for("payments.view_payment",payment_id=payment.id))
    except Exception as e:
        db.session.rollback()
        if _wants_json(): return jsonify(status="error",message=str(e)),400
        flash(f"❌ خطأ أثناء تسجيل الدفع: {e}","danger"); return render_template("payments/form.html",form=form,entity_info=entity_info)
...
def view_payment(payment_id):
    ...
    if _wants_json(): return jsonify(payment=_serialize_payment(payment,full=True))
...
def delete_split(split_id):
    ...
    try:
        ...
        if pmt and pmt.status==PaymentStatus.COMPLETED.value: return jsonify(status="error",message="لا يمكن تعديل دفعات سند مكتمل."),409
        db.session.delete(split); db.session.flush()
        if payment_id:
            if pmt is None: pmt=db.session.get(Payment,payment_id)
            if pmt is not None:
                _sync_payment_method_with_splits(pmt)
                rem=list(pmt.splits or [])
                if rem: pmt.total_amount=_sum_splits_decimal(rem)
                db.session.add(pmt)
        db.session.commit()
...
def view_receipt(payment_id:int):
    ...
    if _wants_json():
        payload=_serialize_payment(payment,full=True); payload["sale_info"]=sale_info; return jsonify(payment=payload)
...
def entity_fields():
    ...
    model_map={"customer":(Customer,"customer_id"),"supplier":(Supplier,"supplier_id"),"partner":(Partner,"partner_id"),"sale":(Sale,"sale_id"),"invoice":(Invoice,"invoice_id"),"service":(ServiceRequest,"service_id"),"shipment":(Shipment,"shipment_id"),"expense":(Expense,"expense_id"),"preorder":(PreOrder,"preorder_id"),"loan":(SupplierLoanSettlement,"loan_settlement_id")}
    if entity_id:
        try:
            eid=int(entity_id)
            model,field_name=model_map.get(entity_type,(None,None))
            if model is not None and db.session.get(model,eid): getattr(form,field_name).data=eid
        except (ValueError,TypeError):
            if _wants_json(): return jsonify(status="error",message="entity_id غير صالح"),400
    if hasattr(form,"_sync_entity_id_for_render"): form._sync_entity_id_for_render()

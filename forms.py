...
class Unique:
    def __init__(self, model, field_name, message=None, case_insensitive=False, normalizer=None):
        self.model = model
    ...
    def __call__(self, form, field):
        ...
        if self.case_insensitive and isinstance(value, str):
            qy = qy.filter(func.lower(column) == value.lower())
        else:
            qy = qy.filter(column == value)
        obj_id_field = getattr(form, "id", None)
        obj_id = obj_id_field.data if obj_id_field is not None else None
        if obj_id and str(obj_id).strip().isdigit():
            qy = qy.filter(self.model.id != int(obj_id))
        if qy.first() is not None:
...
class RoleForm(FlaskForm):
    ...
    def validate_name(self, field):
        ...
        field.data = s
        if Role.query.filter(Role.name == s).first():
...
class PaymentForm(FlaskForm):
    ...
    def validate(self, extra_validators=None):
        ...
        if m == "card":
            ...
            if (self.card_holder.data or "").strip() == "" and num: self.card_holder.errors.append("أدخل اسم حامل البطاقة."); return False
            exp = (self.card_expiry.data or "").strip()
            if exp and not is_valid_expiry_mm_yy(exp): self.card_expiry.errors.append("تاريخ الانتهاء بصيغة MM/YY."); return False
            if num and not (self.card_cvv.data or "").strip(): self.card_cvv.errors.append("أدخل CVV."); return False
        return True

class PreOrderForm(FlaskForm):
    reference      = StringField('مرجع الحجز', validators=[Optional(), Length(max=50)])
    preorder_date  = UnifiedDateTimeField(
        'تاريخ الحجز',
        format='%Y-%m-%d %H:%M',
        validators=[Optional()],
        render_kw={'autocomplete': 'off', 'dir': 'ltr'},
    )
    expected_date  = UnifiedDateTimeField(
    ...
    # تحويل الأرقام العربية وإرجاع int آمن
    def _to_int(self, v):
        try:
            return int(str(v).translate(_AR_DIGITS).strip())
        except Exception:
    ...
    def validate(self, **kwargs):
        ...
        # تحقق من أن تاريخ التسليم المتوقع لا يسبق تاريخ الحجز
        if self.preorder_date.data and self.expected_date.data:
            if self.expected_date.data < self.preorder_date.data:
                self.expected_date.errors.append('❌ تاريخ التسليم المتوقع يجب أن يكون بعد تاريخ الحجز')
                return False

        # توحيد طريقة الدفع (افتراضيًا نقدي إذا تُركت فارغة)
        pm = (self.payment_method.data or '').strip().lower()
        if pm and pm not in {'cash', 'card', 'bank', 'cheque'}:
            self.payment_method.errors.append('❌ طريقة الدفع غير معروفة')
            return False
        if not pm:
            self.payment_method.data = 'cash'
    ...
    def apply_to(self, preorder: PreOrder) -> PreOrder:
        ...
        preorder.payment_method = (self.payment_method.data or 'cash').lower()
...
class ServiceRequestForm(FlaskForm):
    ...
    def validate(self, **kwargs):
        ...
        if st and et and et < st:
            self.end_time.errors.append('❌ وقت الانتهاء يجب أن يكون بعد وقت البدء'); return False
        ra, sa = self.received_at.data, self.started_at.data
        if ra and sa and sa < ra:
            self.started_at.errors.append('❌ وقت البدء الفعلي يجب أن يكون بعد وقت الاستلام'); return False
        if st and sa and sa.date() < st:
            self.started_at.errors.append('❌ وقت البدء الفعلي يجب أن يكون في أو بعد تاريخ البدء التخطيطي'); return False
        ed = self.expected_delivery.data
        if sa and ed and ed < sa:
            self.expected_delivery.errors.append('❌ التسليم المتوقع يجب أن يكون بعد وقت البدء الفعلي'); return False
        if st and ed and ed.date() < st:
            self.expected_delivery.errors.append('❌ التسليم المتوقع يجب أن يكون بعد تاريخ البدء'); return False
        ct = self.completed_at.data
        if sa and ct and ct < sa:
            self.completed_at.errors.append('❌ الإكمال يجب أن يكون بعد وقت البدء الفعلي'); return False
        if st and ct and ct.date() < st:
            self.completed_at.errors.append('❌ الإكمال يجب أن يكون بعد تاريخ البدء'); return False
        parts, labor, disc = self._d(self.parts_total.data), self._d(self.labor_total.data), self._d(self.discount_total.data)
        base = self._q2(parts + labor - disc if (parts + labor - disc) >= 0 else 0)
        taxr = self._d(self.tax_rate.data)
        if taxr < 0 or taxr > self._d(100):
            self.tax_rate.errors.append('❌ نسبة الضريبة يجب أن تكون بين 0 و 100'); return False
        ...
        if self.total_cost.data is None:
            ...
            self.total_cost.errors.append('❌ التكلفة النهائية لا يمكن أن تكون أقل من (قطع + أجور − خصم)'); return False
...
class ShipmentForm(FlaskForm):
    ...
    status           = SelectField('الحالة', choices=[('PENDING','PENDING'), ('IN_TRANSIT','IN_TRANSIT'), ('ARRIVED','ARRIVED'), ('CANCELLED','CANCELLED')], validators=[DataRequired()])
    ...
    def apply_to(self, shipment):
        ...
        shipment.partners = [
            ...
            for f in (entry.form for entry in self.partners) if f.partner_id.data
        ]
        return shipment
...
class ExpenseTypeForm(FlaskForm):
    ...
    def validate_name(self, field):
        name = (field.data or "").strip()
        q = ExpenseType.query.filter(func.lower(ExpenseType.name) == name.lower())
...
class ExpenseForm(FlaskForm):
    ...
    employee_id  = AjaxSelectField('الموظف', validators=[Optional()])
    warehouse_id = AjaxSelectField('المستودع', validators=[Optional()])
    partner_id   = AjaxSelectField('الشريك', validators=[Optional()])
    ...
    def validate(self, extra_validators=None):
        ...
        if m == 'cheque':
            if not self.check_number.data: self.check_number.errors.append('❌ أدخل رقم الشيك'); return False
            if not self.check_bank.data: self.check_bank.errors.append('❌ أدخل اسم البنك'); return False
            if not self.check_due_date.data: self.check_due_date.errors.append('❌ أدخل تاريخ الاستحقاق'); return False
        ...
        elif m == 'bank' and not self.bank_transfer_ref.data:
            self.bank_transfer_ref.errors.append('❌ أدخل مرجع التحويل البنكي'); return False
        elif m == 'card':
            raw = only_digits(self.card_number.data or '')
            if not (raw.isdigit() and luhn_check(raw)): self.card_number.errors.append('❌ رقم البطاقة غير صالح'); return False
            if not self.card_holder.data: self.card_holder.errors.append('❌ أدخل اسم حامل البطاقة'); return False
            if self.card_expiry.data and not is_valid_expiry_mm_yy(self.card_expiry.data):
                self.card_expiry.errors.append('❌ تاريخ الانتهاء غير صالح'); return False
        elif m == 'online':
            if not self.online_gateway.data: self.online_gateway.errors.append('❌ أدخل بوابة الدفع'); return False
            if not self.online_ref.data: self.online_ref.errors.append('❌ أدخل مرجع العملية'); return False
        return True
    ...
    def build_payment_details(self):
        ...
        if self.payment_details.data: details['extra'] = self.payment_details.data
...
class OnlinePaymentForm(FlaskForm):
    ...
    currency         = SelectField('العملة',
                                   choices=[('ILS','ILS'),('USD','USD'),('EUR','EUR'),('JOD','JOD')],
                                   default='ILS', validators=[DataRequired()])
    ...
    status           = SelectField('حالة المعاملة',
                                   choices=[('PENDING','قيد الانتظار'),('COMPLETED','مكتملة'),
                                            ('FAILED','فاشلة'),('REFUNDED','مُرجعة')],
    ...
    def validate(self, **kwargs):
        ...
        if st in {"COMPLETED", "REFUNDED"} and not self.processed_at.data:
...
class ProductForm(FlaskForm):
    ...
    sku = StringField('SKU', validators=[
        Optional(),
        Length(max=50),
        Unique(Product, 'sku', message='SKU مستخدم بالفعل.',
    ...
    serial_no = StringField('الرقم التسلسلي', validators=[
        Optional(),
        Length(max=100),
        Unique(Product, 'serial_no', message='الرقم التسلسلي مستخدم بالفعل.',
               case_insensitive=True,
               normalizer=lambda v: (v or '').strip().upper())
    ])
    barcode = StringField('الباركود', validators=[
        Optional(),
        Length(max=100),
        Unique(Product, 'barcode', message='الباركود مستخدم بالفعل.',
               case_insensitive=True,
...
class WarehouseForm(FlaskForm):
    ...
    parent_id = AjaxSelectField('المستودع الأب', validators=[Optional()])
    partner_id = AjaxSelectField('الشريك', validators=[Optional()])
    supplier_id = AjaxSelectField('المورد', validators=[Optional()])
...
class PartnerShareForm(FlaskForm):
    partner_id = AjaxSelectField(
        'الشريك',
        endpoint='api.search_partners',
        get_label='name',
        validators=[DataRequired()]
    )
...
class AccountForm(FlaskForm):
    ...
    code = StringField('كود الحساب', validators=[
        ...
        Unique(Account, 'code', message='الكود مستخدم مسبقًا')
    ])
    name = StringField('اسم الحساب', validators=[DataRequired(), Length(max=120)])
    type = SelectField(
        'النوع',
        validators=[DataRequired()],
        choices=[(t.value, t.value) for t in AccountType]
    )
    parent_id = AjaxSelectField(
        'الحساب الأب',
        endpoint='api.search_accounts',
        get_label='name',
        allow_blank=True,
        validators=[Optional()]
    )
    ...
    def apply_to(self, acc: Account) -> Account:
        acc.code = (self.code.data or '').strip()
...
class JournalLineForm(FlaskForm):
    ...
    entity_type = SelectField(
        ...
        choices=[
            ...
            ('PARTNER', 'PARTNER'), ('INVOICE', 'INVOICE'), ('SALE', 'SALE'),
...
class GeneralLedgerFilterForm(FlaskForm):
    account_ids = AjaxSelectMultipleField(
        'الحسابات',
        endpoint='api.search_accounts',
        get_label='name',
        validators=[Optional()]
...
class ClosingEntryForm(FlaskForm):
    ...
    revenue_accounts = AjaxSelectMultipleField(
        'حسابات الإيرادات',
        endpoint='api.search_accounts',
        get_label='name',
        validators=[Optional()]
    )
    expense_accounts = AjaxSelectMultipleField(
        'حسابات المصاريف',
        endpoint='api.search_accounts',
        get_label='name',
        validators=[Optional()]
    )
    retained_earnings_account = AjaxSelectField(
        'حساب الأرباح المحتجزة / النتائج',
        endpoint='api.search_accounts',
        get_label='name',
        validators=[DataRequired()]
    )
...
class ExportContactsForm(FlaskForm):
    customer_ids = AjaxSelectMultipleField(
        'اختر العملاء',
        endpoint='api.search_customers',
        get_label='name',
    ...
    format = SelectField(
        'صيغة التصدير',
        choices=[('vcf', 'vCard'), ('csv', 'CSV'), ('excel', 'Excel')],
        default='vcf',
        validators=[DataRequired()],
        coerce=str,
    )

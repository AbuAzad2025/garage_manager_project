...
from utils import qr_to_base64, format_currency, format_percent, format_date, format_datetime, yes_no, status_label, init_app as utils_init_app
...
def inject_helpers():
    def has_perm(code: str) -> bool:
        return current_user.is_authenticated and getattr(current_user, "has_permission", lambda c: False)(code)
...
def create_app(config_object=Config, test_config=None) -> Flask:
    ...
    socketio.init_app(app, async_mode=app.config.get("SOCKETIO_ASYNC_MODE", "threading"), message_queue=app.config.get("SOCKETIO_MESSAGE_QUEUE"))
    mail.init_app(app)
    utils_init_app(app)
    _init_sentry(app)
    ...
    extra_template_paths = [os.path.join(app.root_path, 'templates'), os.path.join(app.root_path, 'routes', 'templates')]
    app.jinja_loader = ChoiceLoader([FileSystemLoader(p) for p in extra_template_paths] + ([app.jinja_loader] if app.jinja_loader else []))
    @app.context_processor
    ...
    for bp in (api_bp, auth_bp, main_bp, users_bp, service_bp, customers_bp, sales_bp, notes_bp, reports_bp, shop_bp, expenses_bp, vendors_bp, shipments_bp, warehouse_bp, payments_bp, permissions_bp, roles_bp):
        app.register_blueprint(bp)
    CORS(app, resources={r"/api/*": {"origins": app.config.get("CORS_ORIGINS", "*"), "supports_credentials": app.config.get("CORS_SUPPORTS_CREDENTIALS", True)}})
    @login_manager.user_loader
    def load_user(user_id):
        from sqlalchemy.orm import joinedload
        try:
            return db.session.get(User, int(user_id), options=[joinedload(User.role).joinedload(Role.permissions)])
        except Exception:
            return None
...
if __name__ == '__main__':
    socketio.run(app, host=os.environ.get('HOST', '0.0.0.0'), port=int(os.environ.get('PORT', 5000)), debug=app.config.get('DEBUG', False))

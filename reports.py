# reports.py — root reports module
...
def sales_report(start_date: date | None, end_date: date | None,
    ...
    engine = db.engine.name  # 'postgresql' | 'sqlite' | 'mysql'...
...
def service_reports_report(start_date: date | None, end_date: date | None) -> dict:
    ...
    completed = db.session.query(func.count(ServiceRequest.id)).filter(
        date_cond, ServiceRequest.status == ServiceStatus.COMPLETED.value
    ).scalar() or 0
    ...
    return {
        ...
        "parts": float(parts or 0),
        "labor": float(labor or 0),
        "data": data,
    }

def ar_aging_report(start_date=None, end_date=None):
    """Accounts Receivable Aging (العملاء)."""
...
def ap_aging_report(start_date=None, end_date=None):
    """Accounts Payable Aging (الموردون)."""
    as_of = _parse_date_like(end_date) or date.today()
    ...
    inv_base = (
        ...
        .filter(~Invoice.status.in_([InvoiceStatus.CANCELLED.value, InvoiceStatus.REFUNDED.value]))
        .filter(cast(ref_date_expr, Date) <= as_of)  # exclude future refs
        .subquery()
    )
...
def top_products_report(
    ...
    # line revenue = qty * unit_price * (1 - disc%) * (1 + tax%)
    value_expr = (
